...
#### <a name="download-error-when-you-try-to-install-a-clickonce-application-that-has-a-config-file"></a>Błąd pobierania, podczas próby zainstalowania aplikacji ClickOnce, który ma pliku config  
 ...
 Aby uzyskać więcej informacji, zobacz "Systemu Windows Server 2003: stref typów zawartości" w [problemy z konfiguracją serwera i klienta we wdrożeniach ClickOnce](../deployment/server-and-client-configuration-issues-in-clickonce-deployments.md).  
